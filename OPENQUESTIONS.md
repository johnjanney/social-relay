...
| OQ-20 | Does X terminate a hashtag at the first character outside `[letter, digit, underscore]`, and does it refuse to linkify an all-digit hashtag? | Claude → John | no | open | **UNVERIFIED 2026-09-05.** Both claims come from X's documented hashtag behaviour and from the widely-observed rendering of hashtags on the platform; neither was measured against the live API, and `bin/verify-x-api.php` does not test them because doing so means publishing real posts. They are the two premises `SPEC.md` §7.6.1 rests on: the first is why punctuation is stripped rather than left in place, and the second is why an all-digit tag produces no hashtag. **Neither is load-bearing.** If the first is wrong, the plugin ships a slightly cleaner hashtag than it needed to; if the second is wrong, it omits a hashtag X would have linked. Neither can overflow 280, fail a send, or affect INV-1. **How to close:** during the Phase 6 staging run, publish one post whose tags include a hyphenated tag and an all-digit tag with the feature on, and read the rendered post on X. |
