...
| OQ-15 | Is the create-post path `/2/tweets` or `/2/posts`? | Claude | no | **resolved** | **RESOLVED 2026-09-06 — owner-reported, in production.** The owner upgraded to 0.3.1 and clicked "Post to X now" on a real post; the post appeared on X. That is a live `POST /2/tweets` call through the plugin's own pipeline, so `SRL_X_Provider::PATH_CREATE = '/2/tweets'` is correct and stays. Owner-reported rather than **[MEASURED]** by a probe: the owner saw the post on the timeline and said so; the HTTP status, the remote id, and the log row were not pasted back. **Still unreported:** whether the featured image was attached (the media path was measured in Phase 0, OQ-2, but not through the publisher on a real post), and whether hashtags were on. *Earlier:* **UNVERIFIED 2026-09-05.** The brief and most secondary sources say `POST /2/tweets`. One search result quoted `POST https://api.x.com/2/posts` for attaching media. The official pricing page names the operation `Post: Create` without giving a path. `bin/verify-x-api.php` does **not** test this, because creating a post costs money and publishes publicly. Resolve at Phase 6 local acceptance via FR-1.5 "Send test post", or by John checking the Console. |
...
- **OQ-15** — **resolved 2026-09-06.** `/2/tweets` created a post on X in production, owner-reported. Listed here because the section below still describes the state at the gate.
