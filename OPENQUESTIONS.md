...
| OQ-15 | Is the create-post path `/2/tweets` or `/2/posts`? | Claude | no | **resolved** | **RESOLVED 2026-09-06 — owner-reported, in production.** The owner upgraded to 0.3.1 and clicked "Post to X now" on a real post; the post appeared on X. That is a live `POST /2/tweets` call through the plugin's own pipeline, so `SRL_X_Provider::PATH_CREATE = '/2/tweets'` is correct and stays. Owner-reported rather than **[MEASURED]** by a probe: the owner saw the post on the timeline and said so; the HTTP status, the remote id, and the log row were not pasted back. **Reported 2026-09-07:** hashtags were on and an image was visible, on both a post sent automatically on publish and one sent by "Post to X now" from the production site. The image is **not** thereby proven attached: the plugin posts the permalink even when the media upload fails (FR-4.4), and X renders an `og:image` link card on its own, so the timeline cannot distinguish the two. The media path through the publisher stays proven only by the Phase 0 probe (OQ-2) until the log row or `_srl_image_omitted` is read. Still not pasted: the HTTP status, remote id and log row. *Earlier:* **UNVERIFIED 2026-09-05.** The brief and most secondary sources say `POST /2/tweets`. One search result quoted `POST https://api.x.com/2/posts` for attaching media. The official pricing page names the operation `Post: Create` without giving a path. `bin/verify-x-api.php` does **not** test this, because creating a post costs money and publishes publicly. Resolve at Phase 6 local acceptance via FR-1.5 "Send test post", or by John checking the Console. |
...
| OQ-19 | Does an App still have to sit inside a **Project**? Brief §1.2 marks this **[VERIFIED]**; the current documentation no longer mentions Projects anywhere. | Claude | no | open | **The brief may be stale here — see "Conflict: OQ-19" below.** **VERIFIED 2026-09-05** that four separate current documentation pages describe an App-centric console with no Project concept: <https://docs.x.com/x-api/getting-started/getting-access> ("Step 1: Create a developer account", "Step 2: Create an app", "Step 3: Save your credentials"), <https://docs.x.com/resources/fundamentals/developer-apps> ("Apps are containers for your API credentials. Each app has its own keys, tokens, and settings."), <https://docs.x.com/resources/fundamentals/developer-portal>, and <https://docs.x.com/fundamentals/authentication/oauth-1-0a/api-key-and-secret>. The console is now at **console.x.com**. **INFERRED**, not verified: the Project requirement was dropped or absorbed in the February 2026 pay-per-use rework. Silence across four pages is strong but is not the same as a statement that Projects are gone; the new console may create one implicitly. Closes when John reports what the console actually shows. **Practical effect: none on the plugin.** It changes only the wording of `INSTALLATION.md` step 2 and the troubleshooting text in `bin/verify-x-api.php`, both of which currently tell the owner to check Project membership when a call fails. **Further evidence 2026-09-05, and the stakes are higher than first recorded.** Two documentation pages were read directly this time rather than inferred from silence. <https://docs.x.com/x-api/getting-started/getting-access> gives a step-by-step walkthrough — "Go to the Developer Console", "Accept the Developer Agreement", "Complete your profile", then "Click 'New App'" — with **no Project step at any point**. <https://docs.x.com/fundamentals/developer-portal> describes the console as "your central hub… Create apps, generate credentials, monitor usage, and manage billing" and **does not mention Projects at all**, presenting Apps as the primary organisational unit. That is a positive App-centric walkthrough, not merely absence of mention, and it strengthens the INFERRED reading. **But it does not close the row, and one search result argues against closing it.** `devcommunity.x.com` carries 2026 threads in which Projects plainly still exist for some accounts — deleting them, renaming them, apps vanishing from them — alongside one titled **"No Projects section in console, POST /2/tweets returns 403"**. The forum returns HTTP 403 to automated fetch, so that thread was **not read**; only its title is evidence, and a title is not a diagnosis. **Why this matters more than the row originally said.** It was recorded as affecting only `INSTALLATION.md`'s wording. If Project membership is in fact still required for write endpoints on some accounts, it would affect **`POST /2/tweets`** — the one call this plugin cannot do without, and the one call Phase 0 never tested because it costs money and publishes publicly (**OQ-15**). OQ-2 proved `GET /2/users/me` and the full media-upload flow at 200 with `x-access-level: read-write`, which makes a credentials or permissions fault unlikely, but it does not prove the create-post endpoint. **Treat OQ-19 and OQ-15 as one check**, settled together the first time FR-1.5 "Send test post" runs against the real API. If that returns 403, this row is the first place to look. **Stakes withdrawn 2026-09-06.** `POST /2/tweets` succeeded on the owner's account in production (OQ-15, resolved). Whatever the console shows, Project membership did not block the one write this plugin needs. The row stays open for what it originally affected — the wording of `INSTALLATION.md` step 2 and the troubleshooting text in `bin/verify-x-api.php` — and closes with one sentence from the owner about whether the console showed a Project. |
| OQ-20 | Does X terminate a hashtag at the first character outside `[letter, digit, underscore]`, and does it refuse to linkify an all-digit hashtag? | Claude | no | **resolved** | **RESOLVED 2026-09-05 by documentation. Both premises confirmed.** X's Help Center article *Help with hashtags and replies* (<https://help.x.com/en/using-x/replies-not-showing-up-and-hashtag-problems>) states that **"punctuation marks (commas, periods, semicolons, apostrophes, question marks, exclamation marks, etc.) will end your hashtag wherever punctuation occurs"**, with the worked example that `#it'sfun` is categorised under **`#it`** — which is exactly the `co-op` → `#co` failure `SPEC.md` §7.6.1 exists to prevent, confirmed by X in X's own words. The same article states **"if you write #1 or #123 the hashtag will not be hyperlinked and is therefore not searchable"** and that **"if you include letters following the numbers in the hashtag, for example, #123go, then the hashtag will work correctly"**. That confirms *both halves* of §7.6.1 rule 5: an all-digit tag produces nothing, and a tag that merely *starts* with digits is kept as long as it contains a letter (`top 10` → `#Top10`, asserted by T-444). **Caveat, recorded rather than glossed.** `help.x.com` returns HTTP 403 to automated fetch, so this is the article's wording as quoted consistently by two independent search passes on 2026-09-05, not a page read first-hand. It is **[DOC]**, not **[MEASURED]**: live rendering of a real post was never observed. Neither claim can overflow 280, fail a send, or affect INV-1, so the Phase 6 staging observation is still worth doing for completeness but is no longer needed to close this row. |
...
- **OQ-15** — **resolved 2026-09-06.** `/2/tweets` created a post on X in production, owner-reported. On 2026-09-07 the owner added that hashtags and link went out and an image was visible, both automatically on publish and by "Post to X now"; whether the image was attached media or X's link card is unverified. Listed here because the section below still describes the state at the gate.
...
- **OQ-19** — whether Apps still sit inside Projects. Closes the moment John says what the console showed. Affects setup wording only, no plugin code — a reading that was withdrawn on 2026-09-05 and **reinstated on 2026-09-06** once the write endpoint succeeded in production.

None of the four blocked writing `SPEC.md`, which was drafted on 2026-09-05. OQ-18 and OQ-19 need one sentence each from John. OQ-1b is a measurement that belongs to a later phase by the brief's own design; OQ-15 was settled by the first real post.
