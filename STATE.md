...
| Image, hashtags, automatic trigger, in production | **yes, owner-reported 2026-09-07** | Publishing an article posted the featured image, title, hashtags and link automatically; "Post to X now" did the same. Response and log row not captured. |
...
| Production post, log row | a **[MEASURED]** record of the create-post response | The image and hashtag questions were answered on 2026-09-07. Still uncaptured: the log row for one production post, which carries the HTTP status and remote id. One paste from the settings page log panel. |
