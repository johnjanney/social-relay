...
| **OPEN-5** | The create-post path is `/2/tweets` or `/2/posts` (OQ-15). Not probed, because it costs money and publishes. | §8.3 | **RESOLVED 2026-09-06 — owner-reported.** `/2/tweets` created a post on X in production via "Post to X now" on 0.3.1. The constant stands. **2026-09-07:** the owner reports the featured image, hashtags and link went out on later production posts, both automatically on publish and by "Post to X now". The response and log row remain uncaptured. |
